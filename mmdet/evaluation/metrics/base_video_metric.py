...
class BaseVideoMetric(BaseMetric):
    ...
    is used. Finally, it computes the metrics of the entire Data.
    ...
    def evaluate(self, size: int = 1) -> dict:
        """Evaluate the model performance of the whole Data after processing
        all batches.

        Args:
            size (int): Length of the entire validation Data.

        Returns:
            dict: Evaluation metrics dict on the val Data. The keys are the
...
def collect_tracking_results(results: list,
    ...
    paramenter size, which means length of the entire validation Data.
