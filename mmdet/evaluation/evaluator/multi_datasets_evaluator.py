...
class MultiDatasetsEvaluator(Evaluator):
    ...
    The metrics will be evaluated on each Data slice separately. The name of
    the each metric is the concatenation of the Data prefix, the metric
    prefix and the key of metric - e.g.
    `dataset_prefix/metric_prefix/accuracy`.

    Args:
        metrics (dict or BaseMetric or Sequence): The config of metrics.
        dataset_prefixes (Sequence[str]): The prefix of each Data. The
    ...
    def evaluate(self, size: int) -> dict:
        ...
        Args:
            size (int): Length of the entire validation Data. When batch
                size > 1, the dataloader may pad some data samples to make
                sure all ranks have the same length of Data slice. The
